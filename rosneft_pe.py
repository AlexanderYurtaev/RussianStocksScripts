"""
Скрипт расчёта текущего P/E Роснефти (ROSN) на Московской бирже.

Источники данных:
- Цена акции: Moex ISS API (автоматически)
- Кол-во акций: Moex ISS API, поле ISSUESIZE (автоматически)
- Чистая прибыль: вводится вручную из отчётов Роснефти
"""
...
MOEX_ISS_URL = (
    "https://iss.moex.com/iss/engines/stock/markets/shares/securities/ROSN.json"
)


def fetch_moex_data():
    """Получить все данные по ROSN с Moex ISS API."""
    try:
...
def main():
    print("=" * 60)
    print("  Роснефть (ROSN) — расчёт P/E")
    ...
    # --- Ручной ввод прибыли ---
    print("Введите чистую прибыль Роснефти из отчётности:")
    print("  (данные IFRS/РСБУ, в млрд RUB)")
    print("  Источник: https://www.rosneft.ru/investors/financial-reports/")
    print()
    ...
    print()
    print("Подсказка: данные о прибыли берите из официальных отчётов Роснефти.")
