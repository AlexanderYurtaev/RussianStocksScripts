"""
Скрипт расчёта текущего P/E Северстали (CHMF) на Московской бирже.

Источники данных:
- Цена акции: Moex ISS API (автоматически)
- Кол-во акций: Moex ISS API, поле ISSUESIZE (автоматически)
- Чистая прибыль: вводится вручную из отчётов Северстали
"""
...
MOEX_ISS_URL = (
    "https://iss.moex.com/iss/engines/stock/markets/shares/securities/CHMF.json"
)


def fetch_moex_data():
    """Получить все данные по CHMF с Moex ISS API."""
    try:
...
def main():
    print("=" * 60)
    print("  Северсталь (CHMF) — расчёт P/E")
    ...
    # --- Ручной ввод прибыли ---
    print("Введите чистую прибыль Северстали из отчётности:")
    print("  (данные IFRS/РСБУ, в млрд RUB)")
    print("  Источник: https://severstal.com/investory/finansovaya-otchetnost/")
    print()
    ...
    print()
    print("Подсказка: данные о прибыли берите из официальных отчётов Северстали.")
